  ...
  useEffect(() => {
    const fetchSchoolAdmins = async () => {
      ...
      if (error) {
        ...
        // Fetch the email from the school_admin_auth table using admin_auth_id
        const formattedData = await Promise.all(
          data?.map(async (admin) => {
            // Fetch the email associated with the admin's auth_id
            ...
            return {
              ...
              email: authData.email, // Set email from the school_admin_auth table
            };
          }) || []
        );
    ...
    // Step 1: Extract access token from localStorage
    const token = localStorage.getItem("access_token");

    if (token) {
      // Step 2: Decode the token to get the email
      const decodedToken = JSON.parse(atob(token.split(".")[1]));
      const email = decodedToken.email;

      // Log the email for debugging
      console.log("Email extracted from JWT token:", email);

      // Step 3: Find the admin based on email
      const matchedAdmin = schoolAdmins.find((admin) => admin.email === email);
      if (matchedAdmin) {
        // Step 4: Set the selected admin id, school name, and class options
        setSelectedAdminId(matchedAdmin.id);
        setSchoolName(matchedAdmin.school_name); // Set school name
        setClassOptions(matchedAdmin.classes || []); // Set class options
      }
    }
  }, [supabase, schoolAdmins]);

  useEffect(() => {
    // This ensures we only set loading to false after fetching and setting school admins
